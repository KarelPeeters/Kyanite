...
use alpha_zero::mapping::chess::{ChessStdMapper, ClassifiedMove};
...
fn queen_direction() {
    ...
    let board = board("8/8/8/6k1/3Q4/6K1/8/8 w - - 0 1");
...
fn knight_direction() {
    ...
    let board = board("8/8/8/6k1/3N4/6K1/8/8 w - - 0 1");
...
fn black_potential_promotions() {
    ...
    test_policy_pairs(&board, &[
        // rook, no promotion
        (Some((4 * 7 + 1) * 64 + Square::C3.to_index()), Some(ChessMove::new(Square::C3, Square::C1, None))),
        (Some((4 * 7 + 0) * 64 + Square::B2.to_index()), Some(ChessMove::new(Square::B2, Square::B1, None))),
        // knight, no promotion
        (Some(59 * 64 + Square::B3.to_index()), Some(ChessMove::new(Square::B3, Square::C1, None))),
        (Some(60 * 64 + Square::B3.to_index()), Some(ChessMove::new(Square::B3, Square::A1, None))),

        // promotion to queen
        (Some((5 * 7 + 0) * 64 + Square::G2.to_index()), Some(ChessMove::new(Square::G2, Square::F1, Some(Piece::Queen)))),
        (Some((4 * 7 + 0) * 64 + Square::G2.to_index()), Some(ChessMove::new(Square::G2, Square::G1, Some(Piece::Queen)))),
        (Some((3 * 7 + 0) * 64 + Square::G2.to_index()), Some(ChessMove::new(Square::G2, Square::H1, Some(Piece::Queen)))),

        // underpromotion
        (Some(67 * 64 + Square::G2.to_index()), Some(ChessMove::new(Square::G2, Square::G1, Some(Piece::Rook)))),
        (Some(70 * 64 + Square::G2.to_index()), Some(ChessMove::new(Square::G2, Square::H1, Some(Piece::Rook)))),
        (Some(64 * 64 + Square::G2.to_index()), Some(ChessMove::new(Square::G2, Square::F1, Some(Piece::Rook)))),
        (Some(68 * 64 + Square::G2.to_index()), Some(ChessMove::new(Square::G2, Square::G1, Some(Piece::Bishop)))),
        (Some(71 * 64 + Square::G2.to_index()), Some(ChessMove::new(Square::G2, Square::H1, Some(Piece::Bishop)))),
        (Some(65 * 64 + Square::G2.to_index()), Some(ChessMove::new(Square::G2, Square::F1, Some(Piece::Bishop)))),
        (Some(69 * 64 + Square::G2.to_index()), Some(ChessMove::new(Square::G2, Square::G1, Some(Piece::Knight)))),
        (Some(72 * 64 + Square::G2.to_index()), Some(ChessMove::new(Square::G2, Square::H1, Some(Piece::Knight)))),
        (Some(66 * 64 + Square::G2.to_index()), Some(ChessMove::new(Square::G2, Square::F1, Some(Piece::Knight)))),
    ])
}
...
fn en_passant() {
    ...
    test_policy_pairs(&black_board, &[
        (Some((3 * 7 + 0) * 64 + Square::B4.to_index()), Some(ChessMove::new(Square::B4, Square::C3, None)))
    ]);
}
...
fn castles() {
    ...
    test_policy_pairs(&black_board, &[
        (Some((2 * 7 + 1) * 64 + Square::E8.to_index()), Some(ChessMove::new(Square::E8, Square::G8, None))),
        (Some((6 * 7 + 1) * 64 + Square::E8.to_index()), Some(ChessMove::new(Square::E8, Square::C8, None))),
    ]);
}

fn board(fen: &str) -> ChessBoard {
    ChessBoard::new(chess::Board::from_str(fen).unwrap(), 0)
}
